...
//            ],//银联支付
//            self::OFFLINEPAY => [
//                'name' => get_lang('enum_pay.type_offline'),
//                'key' => self::OFFLINEPAY,
...
//            self::OFFLINEPAY => get_lang('enum_pay.type_offline'),//线下支付
//            self::BALANCEPAY => get_lang('enum_pay.type_balancepay'),//余额支付
        ];
        if(!empty($types)){
            foreach($list as $k => $v){
                if(!in_array($k, $types)){
    ...
    public static function getStatus(){
        $list = [
            self::STATUS_WAIT => get_lang('enum_pay.status_wait'),
            self::STATUS_ING => get_lang('enum_pay.status_ing'),
            self::STATUS_ED => get_lang('enum_pay.status_ed'),
            self::STATUS_CALCLE => get_lang('enum_pay.status_cancle'),
        ];
        return $list;
    }
