			...
			<div class="preview-wrap flex-1 relative mt-[20px]">
				...
				<el-scrollbar>
					...
					<div class="diy-view-wrap w-[375px] shadow-lg mx-auto">
						<div class="preview-head bg-no-repeat bg-center bg-cover"
    ...
    initPage({
        ...
        if (data.addon_list) {
            ...
            for (let i = 0; i < data.addon_list.length; i++) {
                let item = data.addon_list[i];
                for (let [key, value] of Object.entries(addonModules)) {
                    let moduleName = key.split('/');
                    let addonName = moduleName[2];
                    if (item.key == addonName) {
                        let name = moduleName[moduleName.length - 1].replace('.vue', '');
                        let module = await value();
