		...
		<div class="main-body flex-1 mr-[15px]" :class="!loading ? '' : 'hidden'">
			<el-card class="box-card !border-none" shadow="never">
				<div class="flex justify-between items-center">
					...
					<div class="flex">
						<div>
							<div><span class="text-[14px] text-[#666]">{{ t('periodTime') }}：{{
								statInfo.site_info.create_time ? statInfo.site_info.create_time.split(' ')[0] : '' }} /
				...
				<div class="mt-[60px]" v-if="!loading">
					<div class="card-header flex justify-between border-b-[1px] border-[#EBEBEC] pb-[15px]">
						...
						<div @click="optionModel" v-if="!edit_menu" class="cursor-pointer">
							<img class="w-[15px]" src="@/app/assets/images/index/edit.png" />
						</div>
						<div v-if="edit_menu">
					...
					<div class="flex flex-wrap">
						<div class="flex flex-wrap" ref="shortcutModel" v-if="edit_menu">
							<div v-for="(items, index) in shortcut_menu" :style="{ backgroundColor: items.bg_color }"
								:key="items.id"
								class="design-field w-[180px] h-[120px] relative mt-[30px] rounded-[5px] mr-[30px] cursor-pointer">
						...
						<div v-else>
							<span class="bg-primary py-[5px] px-[10px] text-[#fff] relative rounded-[3px]">
								...
								<span class="absolute top-[-5px] right-[-5px] cursor-pointer" @click="deleteMenu">
									<img class="w-[15px] h-[15px]" src="@/app/assets/images/index/del_model.png" />
								</span>
							</span>
						</div>
					</div>
...
import { getShortcutMenu, setShortcutMenu, getAuthMenu } from '@/app/api/sys'
...
const statInfo = ref({ 'today_data': {}, system: {}, version: {}, about: [], visit_stat: {}, member_stat: {}, site_info: {}, total_data: {} })
...
const optionModel = () => {
    ...
    setTimeout(() => {
        const sortable = Sortable.create(shortcutModel.value, {
            ...
            animation: 200,
            onEnd: (evt) => {
                const arr = shortcut_menu.value[evt.oldIndex!]
