        ...
        <el-card class="box-card !border-none" shadow="never">
            ...
            <el-card class="box-card !border-none my-[16px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="memberTableData.searchParam" ref="searchFormRef">
                    ...
                    <el-form-item :label="t('registerType')" prop="register_type">
                        <el-select v-model="memberTableData.searchParam.register_type" clearable :placeholder="t('channelPlaceholder')" class="input-width">
                            <el-option :label="t('selectPlaceholder')" value="" />
                            <el-option :label="item" :value="key" v-for="(item,key) in channelList" />
                        </el-select>
                    </el-form-item>
                    <el-form-item :label="t('memberLabel')" prop="member_label">
                        <el-select v-model="memberTableData.searchParam.member_label"  collapse-tags :placeholder="t('memberLabelPlaceholder')" class="input-width">
                            <el-option :label="item['label_name']" :value="item['label_id']" v-for="item in labelSelectData" />
                        </el-select>
                    </el-form-item>
                    <el-form-item :label="t('createTime')" prop="create_time">
                        <el-date-picker
                            v-model="memberTableData.searchParam.create_time"
                            type="datetimerange"
            ...
            <div class="mt-[16px]">
                <el-table :data="memberTableData.data" size="large" v-loading="memberTableData.loading">
                    ...
                    <el-table-column prop="point" :label="t('point')" min-width="100">
                        <template #default="{ row }">
                            {{Number.parseInt(row.point)}}  
                        </template>
                    </el-table-column>
                    <el-table-column prop="balance" :label="t('balance')" min-width="100" />
                    <el-table-column prop="member_label" :label="t('lable')" min-width="120" >
                        <template #default="{ row }">
                            {{memberLable(row)}}  
                        </template>
                    </el-table-column>
                    <el-table-column prop="register_type_name" :label="t('registerType')" min-width="120" />
                    <el-table-column :label="t('createTime')" min-width="120" align="center">
                        <template #default="{ row }">
                            {{ row.create_time || '' }}
                        </template>
                    </el-table-column>
                    <el-table-column :label="t('lastVisitTime')" min-width="120" align="center">
                        <template #default="{ row }">
                            {{ row.update_time || '' }}
                        </template>
                    </el-table-column>
...
import { addMember, getRegisterType, getMemberList,getMemberLabelAll } from '@/api/member'
...
const setChannelList = async () => {
    channelList.value = await (await getRegisterType({})).data
}
setChannelList();
