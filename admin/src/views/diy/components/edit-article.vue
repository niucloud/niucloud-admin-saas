<template>
	<div class="edit-article">
		...
		<el-form label-width="80px" class="px-[10px]">
			...
			<el-form-item :label="t('articleNum')" v-show="diyStore.editComponent.sources == 'initial'">
				<el-slider v-model="diyStore.editComponent.count" show-input size="small"
				           class="ml-[10px] article-slider"
				           :min="1" :max="30"/>
			</el-form-item>
			<el-form-item :label="t('manualSelectionSources')" v-show="diyStore.editComponent.sources == 'diy'"
			              class=" flex">
				<span @click="showArticle" class="cursor-pointer flex-1"
				      :class="{ 'text-primary' : diyStore.editComponent.articleIds.length > 0 }">{{ diyStore.editComponent.articleIds.length > 0 ? t('selected') + diyStore.editComponent.articleIds.length + t('piece') : t('selectPlaceholder') }}</span>
				<el-icon>
					<ArrowRight/>
				</el-icon>
			</el-form-item>
	...
	<el-dialog v-model="showDialog" :title="t('selectArticleTips')" width="60%" :close-on-press-escape="false"
